  ...
  agent any
    stages {
            ...
            steps {
              script{
                if(env.GIT_BRANCH == 'master') {
                  sh 'docker build -t  drivehub.azurecr.io/master:${BUILD_NUMBER} .'
                  sh 'docker push  drivehub.azurecr.io/master:${BUILD_NUMBER}'
                }
                else if(env.GIT_BRANCH == 'develop') {
                  sh 'docker build -t  drivehub.azurecr.io/develop:develop .'
                  sh 'docker push  drivehub.azurecr.io/develop:develop'  
                }
              } 
            }
          }     
      ...
      stage('run unit tests') {   
        ...
        post {
          always {
            discordSend description:'*Build:' + " " + env.BUILD_NUMBER + '\n **Branch:' + " " + env.GIT_BRANCH + '\n **Status:' + " " +  currentBuild.result + '\n \n \n **Commit ID:'+ " " + env.GIT_SHORT_COMMIT + '\n **commit massage:' + " " + env.GIT_COMMIT_MSG + '\n **commit email*:' + " " + env.GIT_COMMITTER_EMAIL, footer: '', image: '', link: 'http://52.164.201.18/blue/organizations/jenkins/'+env.JOB_NAME+'/detail/'+env.JOB_NAME+'/'+env.BUILD_NUMBER+'/pipeline', result: currentBuild.result, thumbnail: '', title: ' link to result', webhookURL: "$discord_token"    
          }
        }
      }
    }   
